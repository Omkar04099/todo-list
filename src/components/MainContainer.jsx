  ...
  return (
    <>
      <div className="flex justify-center h-1/6">
        <div>
          <input onChange={(e) => setNewToDo(e.target.value)} value={newTodo} type="text" placeholder='Enter a Todo' className='w-2/3 px-5 py-1 rounded-lg' style={{border:'2px solid black', fontSize:'1.4rem'}} />
          {toggleBtn ?
            <button onClick={editItem} className=' px-2 py-1 mx-2 rounded-md' style={{backgroundColor:'lightgreen', fontSize:'1.4rem'}}>Update</button> :
            <button onClick={addItemToDo} className='px-2 py-1 mx-2 rounded-md' style={{backgroundColor:'lightgreen', fontSize:'1.4rem'}}>Add</button>
          }
        </div>
      </div>
